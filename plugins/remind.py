...
def say_tell(user, channel):
    cursor = yui.db.execute("""\
        ...
        WHERE nick=? COLLATE NOCASE AND channel=? COLLATE NOCASE""", (user.nick, channel))
    rows = cursor.fetchall()
    if len(rows) < 1:
        return
    cursor = yui.db.execute("""\
        DELETE FROM tell WHERE nick=? COLLATE NOCASE AND channel=? COLLATE NOCASE""", (user.nick, channel))
    yui.db.commit()
